...
**Zeeman (stand 10-08):** categoriepagina's hydrateren niet — óók niet in een echte browser met consent en scrollen — en `/api/*` is per robots.txt verboden (dat respecteren we). De productpagina's laden wél gewoon; met de verbeterde extractie is de sitemap-route (`/sitemap/index.xml` uit robots.txt) de laatste serieuze kans. Werkt die niet, dan is Zeeman eerlijk rood met de weekfolder als alternatief voor prijspeiling.
