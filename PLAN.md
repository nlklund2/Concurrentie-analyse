...
### Toegangsladder (14-09): een bron die dichtgaat kost geen week meer
Zeeman ging tussen maandag 07-09 (W37 groen, 1.058 artikelen) en maandag 14-09 dicht: HTTP 403 op élke request vanaf GitHub Actions, vanaf de allereerste seed, in beide runs van die ochtend. Aan onze kant was niets veranderd — de bron had een poortwachter aangezet. Dat is de derde keer dat een bron van de ene op de andere week omslaat (Action 03-09: sessiewering; Wibra/HEMA: datacenter-IP), en elke keer betekende het een week zonder cijfers plus handwerk. Het structurele antwoord is de **toegangsladder** (`scraper/fetch.py`): per bron een volgorde van clients, van goedkoop naar zwaar — `http` (requests), `chrome` (curl_cffi met het TLS-/HTTP2-handschrift van een echte Chrome), `browser` (Playwright, lost JavaScript-challenges op), `firecrawl` (residentieel IP; betaald en gecapt). De crawl begint onderaan en klimt pas bij een aantoonbare weigering (403/429 of een challenge-pagina); de trede die het werk deed staat in het weekrapport (`listing+chrome`), zodat een verschuiving zichtbaar is vóór hij een probleem wordt. De diagnose meet bij een 403 dezelfde pagina via elke trede (toegangsmatrix) en noemt de poortwachter bij naam (`scraper/blokkade.py`). **Uitkomst voor Zeeman (14-09):** de 403 komt van CloudFront ("Request blocked") en treft álle clients vanaf een datacenter-IP — ook een echte Chromium en Firecrawl's standaardproxy; alleen een residentieel IP (Firecrawl `proxy: enhanced`) krijgt de pagina, ongewijzigd, met dezelfde flight-payload als op 04-09. Het is dus een IP-wering, geen fingerprint of challenge; Zeeman draait daarom op de trede `firecrawl` met `firecrawl_proxy: enhanced` (betaald; ±50 opvragingen per week, gecapt op 60). Wat níet verandert: ±1 request per seconde, robots.txt wordt gerespecteerd (opgehaald via dezelfde trede), alleen publieke lijstpagina's, één run per week. Een donderdagse kanarie (workflow "Toegangscheck bronnen") faalt zodra een bron rood is, zodat er nog dagen zijn om bij te stellen. Fair-use-positie: de ladder omzeilt een *techniek*-filter (welke client vraagt), niet een toegangsregel (robots.txt, login, betaalmuur); de Firecrawl-trede valt onder de proxy-afweging hierboven. Meting en besluit: `docs/validaties/2026-09-14-zeeman-toegangsladder.md`.
